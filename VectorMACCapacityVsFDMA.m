...
for ll=1:2
    ...
    for k=1:length(Ptx)
        ...
        C = [[0;C2], [Csum-C2;C2], [C1;Csum-C1], [C1;0]];
        ...
        num_temp = P(1,1)*norm(H(:,1))^2;
        dem_temp = num_temp + (P(1,2)*norm(H(:,2))^2);
        alpha_opt = num_temp/dem_temp;
        R_opt = FDMArates(H,P,alpha_opt);
        plot(R_opt(1),R_opt(2),'o');
        hold off
